(function () {
    ...
    addMessageListener('patent:swParams', function (m) {
        ...
        try {
            ...
            var body =
                'an=' + encodeURIComponent(m.data.an) + '&pubType=' + (m.data.pt || '3') + '&ggr=' + (m.data.ggr || '');
            ...
            x.onload = function () {
                ...
                sendAsyncMessage('patent:navStatus', {
                    stage: 'adv_resp',
                    status: x.status,
                    len: resp.length,
                    preview: resp.substring(0, 600),
                });
                var fm = resp.match(/https?:\/\/egaz\.cnipa\.gov\.cn[^"'\s]*/);
                if (fm) sendAsyncMessage('patent:pdf', { url: fm[0] });
                else {
                    var pm = resp.match(/(filedl|showpdf)[^"'\s]*/);
                    if (pm) sendAsyncMessage('patent:pdf', { url: 'http://egaz.cnipa.gov.cn/' + pm[0] });
                }
            };
            x.onerror = function () {
                sendAsyncMessage('patent:navStatus', { stage: 'xhr_err' });
            };
            x.send(body);
        } catch (e) {
            sendAsyncMessage('patent:navStatus', { stage: 'swParams_err', msg: e.message });
        }
    });
    var Si = function (f, d) {
        return content.setInterval(f, d);
    };
    var Ci = function (i) {
        return content.clearInterval(i);
    };
    addMessageListener('patent:auto', function (m) {
        var k = m.data.keyword;
        if (!k) return;
        try {
            ...
            if (!ii) {
                sendAsyncMessage('patent:status', { stage: 'err', msg: 'Ti not found' });
                return;
            }
            ii.value = k;
            ii.dispatchEvent(new content.Event('input', { bubbles: true }));
            ii.dispatchEvent(new content.Event('change', { bubbles: true }));
            var sb = content.document.querySelector(
                '.search-btn,.btn-search,input[type=submit],button[type=submit],button',
            );
            if (!sb) {
                sendAsyncMessage('patent:status', { stage: 'err', msg: 'sbtn not found' });
                return;
            }
            sb.click();
                ...
                try {
                    ...
                    ze.forEach(function (el) {
                        ...
                        if (p[0]) {
                            ...
                            if (
                                !content._savedZl.some(function (x) {
                                    return x.an === an;
                                })
                            ) {
                                content._savedZl.push({
                                    an: an,
                                    pt: (p[1] || '').replace(/'/g, ''),
                                    ggr: (p[2] || '').replace(/'/g, ''),
                                });
                    ...
                    if (content._savedZl.length > 0 || c > 20) {
                        return;
                    }
                } catch (e) {}
    ...
    function stepDL() {
        ...
        try {
            ...
            var tmr = Si(function () {
                ...
                if (wc > 30) {
                    Ci(tmr);
                    sendAsyncMessage('patent:status', { stage: 'err', msg: 'dl timeout' });
                    return;
                }
                try {
                    ...
                    Ci(tmr);
                    dl.click();
                    sendAsyncMessage('patent:status', { stage: 'dl_clicked' });
                    var w2 = 0;
                    var t2 = Si(function () {
                        w2++;
                        if (w2 > 20) {
                            Ci(t2);
                            sendAsyncMessage('patent:status', { stage: 'err', msg: 'captcha timeout' });
                            return;
                        }
                        try {
                            ...
                            if (ai) {
                                ...
                                for (var oi = 0; oi < oks.length; oi++) {
                                    ...
                                    if (ot.indexOf('确定') >= 0) {
                                        oks[oi].click();
                                        sendAsyncMessage('patent:status', { stage: 'captcha_ok' });
                                        break;
                                    }
                                }
                            }
    ...
    addMessageListener('patent:doNav', function (m) {
        try {
            if (content._savedZl && content._savedZl.length > 0) {
                ...
                f.method = 'POST';
                f.action = 'http://epub.cnipa.gov.cn/Sw/SwDetail';
                f.style.display = 'none';
                var fn = function (n, v) {
                    var i = content.document.createElement('input');
                    i.type = 'hidden';
                    i.name = n;
                    i.value = v;
                    f.appendChild(i);
            ...
            for (var oi = 0; oi < ons.length; oi++) {
                ...
                var el = ons[oi];
                var ctx = '';
                for (var ci = 0; ci < 5 && el.parentElement; ci++) {
                    el = el.parentElement;
                    ctx = tr2(el.textContent || '');
                }
            ...
            if (targetBtn) {
                ...
                sendAsyncMessage('patent:navStatus', {
                    ...
                    pt: pt2,
                    ggr: ggr2,
                    url: content.location.href.substring(0, 60),
                });
                return;
            }
